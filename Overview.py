...
def createSubjectMarkTrends(data,level,data_store,title):
    ...
    for index, row in data.iterrows():
        ...
        ax.set_xticklabels([2017,2018,2019,2021,2022])
        plt.legend()
        pdf.savefig()
        plt.close()
    pdf.close()
...
#     plot.title.set_text('School % - National % Grades')
#     school_per = []
#     national_per = []
...
def makeComponentTable(school,national,plot):
    ...
    table = plot.table(cellText=data.values, colLabels=data.columns,rowLabels = row_labels, loc='center',fontsize=24)
...
# specialMaths(year,75,institution)
# specialMaths(year,76,institution)
