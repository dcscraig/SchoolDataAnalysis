...
def createSubjectMarkTrends(years,data,level,data_store,title):
    ...
    for index, row in data.iterrows():
        ...
        ax.set_title(index)
        #max
        max_val = row[0:len(years)].values
        # ax.plot(100*(row[0:4].values/max_val),ls='--',label="Max")
        grade_display = ["A","B","C","D"]
        grade_count = 0
        for i in range(len(years),(len(years)*4)+1,len(years)):
            print(row[i:i+len(years)])
            ax.plot(100*(row[i:i+len(years)].values/max_val),ls='--',marker='*',label=grade_display[grade_count])
            grade_count +=1
        ...
        for year in years:
            ...
            count +=1
        ax.set_xticks(numpy.arange(0,len(years)))
